...
class PaperSearch(BaseModel):
    ...
    max_papers: Optional[int] = Field(None, description="Maximum number of papers to return")


class PaperAdd(BaseModel):
    """Model for adding a paper."""
    path: str = Field(..., description="Path to the paper file or URL")
    citation: Optional[str] = Field(None, description="Optional citation for the paper")


class DirectoryIndex(BaseModel):
    """Model for indexing a directory of papers."""
    directory: Optional[str] = Field(None, description="Directory containing papers to index")


# Dependency for getting the PaperQA service
...
async def query_papers(
    query: PaperQuery,
    service: PaperQAService = Depends(get_paperqa_service)
):
    """
